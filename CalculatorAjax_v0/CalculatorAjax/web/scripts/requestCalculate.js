		...
		// verifica della risposta da parte del server
	        if ( theXhr.status === 200 ) {
		        ...
		        if ( theXhr.responseText && theXhr.responseText !== "" ) {
				        callback(JSON.parse(theXhr.responseText).res);
				}
 ...
 */
function requestCalculateAJAX(theUri, theXhr, callback) {
 ...
 */
function requestCalculate(uri,callback) { //passo l'uri della servlet e la set state

	// assegnazione oggetto XMLHttpRequest
	var xhr = myGetXmlHttpRequest();

	if ( xhr ) 
		requestCalculateAJAX(uri,xhr, callback); 
	else 
